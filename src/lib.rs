...
mod test {
    ...
    use config::{
        ...
        GKRConfig, GKRScheme, MPIConfig,
    };
    use gkr::{Prover, Verifier};
    use rand::Rng;
    use sha2::Digest;

    const KECCAK_GF2_CIRCUIT: &str = "data/circuit.txt";
    const KECCAK_M31_CIRCUIT: &str = "data/circuit.txt";
    const KECCAK_BN254_CIRCUIT: &str = "data/circuit.txt";

    const KECCAK_GF2_WITNESS: &str = "data/witness.txt";
    const KECCAK_M31_WITNESS: &str = "data/witness.txt";
    const KECCAK_BN254_WITNESS: &str = "data/witness.txt";

    // RUSTFLAGS="-C target-cpu=native" cargo test --release --package ecc-test --lib -- test::test_proof --exact --show-output
    #[test]
    fn test_proof() {
        ...
        test_gkr_correctness_helper::<BN254ConfigMIMC5>(
            ...
            Some("/Users/5/ecc-test/data/gkr_proof.txt"),
        );
